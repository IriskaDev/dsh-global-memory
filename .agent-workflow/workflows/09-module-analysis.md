...
- 项目为单包插件（非 monorepo），当前 modules/ 尚无业务模块档案（index 表为空）。
- 首次全量分析建议产出 1 个顶层单模块 global-memory，再同步 modules/index.md。
...
| 结构类型             | 判定依据                                                                    | 本项目的模块边界                                                                                            |
| -------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| 扁平结构（单包插件） | package.json 无 workspaces；src/ 下 3 个文件直接平铺，无功能子目录/分层目录 | 整体作为单一模块：global-memory = src/index.ts（插件面）+ src/store.ts（存储面）+ src/store.test.ts（测试） |

- 暂不需要 Group 分层（子模块数量 < 3）；若后续 index/store 各自扩展出独立职责，再评估拆分为 Group。
...
- 推荐文档路径：.agent-workflow/modules/global-memory.md（顶层单模块，类型 MODULE）。
- 归档后立即更新 modules/index.md：模块ID=global-memory，关键词如 global-memory,memory,tool,command,store（≤8 个）。
- 若未来拆分，子模块放 modules/<group>/<sub>.md，禁止超过 2 层。
...
| 路径                         | 分析要点                                                                                                        |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------- |
| src/index.ts                 | 入口与对外接口：export name / inject / apply；4 个工具 + 2 个命令 + systemPrompt.context                        |
| src/store.ts                 | 核心数据结构与接口：MemoryIndex/MemoryRecord/saveMemory/readMemory/searchMemories/deleteMemory/resolveMemoryDir |
| src/store.test.ts            | 12 个用例覆盖解析、白名单、往返、覆盖、搜索、索引重建                                                           |
| package.json / tsconfig.json | 包边界与编译边界                                                                                                |
| scripts/build.sh             | 构建辅助脚本                                                                                                    |
| .agent-workflow/modules/     | 模块档案输出目录（当前仅模板与空索引）                                                                          |

- 外部交互点：仅本机文件系统 $DSH_HOME/memory/；无 DB/HTTP/MQ。
...
不适用：package.json 无 workspaces；未检测到 lerna/nx/turbo/pnpm-workspace 等配置。
...
- .agent-workflow/templates/module-template.md（档案模板）
- .agent-workflow/modules/index.md（顶层索引，分析后必须更新）
...
- 本流程是操作 SOP；实际模块档案尚未生成，待用户触发“分析所有业务模块”或“分析 global-memory 模块”。
- 09 不计入阶段一完善度评分。
